...
- Python 基础模块使用 `<project_prefix>` 包路径，例如 `src/<project_prefix>/modules/`
- Python 主入口固定为项目根目录下的 `main.py`
...
在这个项目里，优先保持“基础能力模块 + 场景编排层 + 单一主入口”三层结构：

- 基础能力模块负责单一职责、可复用的处理能力。
- 场景编排层负责围绕某类需求组织处理步骤。
- `main.py` 负责统一启动、配置接入和界面初始化。
...
- 项目根目录下的 `.py` 文件
  - `main.py` 是唯一应用入口
  - `logging_config.py` 是统一日志基础设施，不属于业务入口
  - 不为不同工作流新增根目录 Python 入口；工作流统一放入 `flows/`
...
- `main.py` 只负责加载配置、初始化日志和界面，并调用 `flows`。
- 不同业务场景通过 `flows` 中的模块区分。
- 公共逻辑应下沉到 `flows` 或 `modules`，不要堆入 `main.py`。

推荐示例：

- `main.py`：唯一应用入口
- `flows/scan_flow.py`：扫描工作流
- `flows/report_flow.py`：报告工作流
- `flows/export_flow.py`：导出工作流
...
- 入口脚本只负责：
  ...
  - 输出结果并处理退出状态
- 不要把核心业务逻辑直接写进入口脚本。
- 新增需求时，在 `flows/` 新增工作流，并从 `main.py` 启动的界面或应用层接入。
- 不要为工作流新增第二个根目录 Python 入口。

### 入口文件头 docstring

项目根目录下的 `main.py` 必须在文件头部写模块级 docstring。
...
5. 不同需求放入根目录 `flows/`，统一由 `main.py` 接入。
...
- 在 `main.py` 中直接堆完整业务逻辑
- 为每个工作流新增根目录 Python 入口
- 在编排层中重复实现底层处理细节
- 在基础模块中混入完整流程编排逻辑
- 在源码里写死本机绝对路径
- 为每个模块单独维护一套日志配置
